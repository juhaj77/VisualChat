...
const UploadForm = ({theme,setVisible,top,left,channelId,user,channel }) => {
  ...
  const onChangeHandler = (event) => {
    ...
    if(files[0] && files[0].size < 4000000 && files[0].name.match(/\.(JPG|jpg|jpeg|png)$/)) {
      setLoaded(0)
  ...
  return (
    <div className={'Container2 '+theme} style={{display:'flex', flexDirection:'column',alignItems: 'center', justifyContent:'center',zIndex:'2000',position:'absolute',top:top,left:left}}>
      <FormHeader className='prevent-select'>
        select picture
      </FormHeader>
      <FlexItem>
        <LabelStyle className='prevent-select'>max size 4M</LabelStyle>
        <input
